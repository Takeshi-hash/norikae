...
2. **駅の追加**: 「到着駅名」に駅名を入力して追加ボタンを押すだけです。
    - ※運賃取得にはジョルダンを利用しており、完了まで約10秒かかります。
3. **AIによるデータ補完**: 駅名だけで登録した後、チャットで私に **「駅を追加したよ」** または **「捕捉しておいて」** と伝えてください。
    - 私が自律的にスキャンして、**「よみがな」** と **「エリア」** を適切な内容で自動設定します（`/maintenance` コマンドでも発動します）。
4. **サーバーの終了**: 作業が終わったら「🛑 作業完了・サーバー終了」ボタンを押して終了してください。
